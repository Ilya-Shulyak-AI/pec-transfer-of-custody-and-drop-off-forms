(() => {
  ...
  function syncReceiverContactName(options = {}) { const select = $('receiverContactName'); const phone = $('receiverPhone'); if (!select || !phone) return; const preservePhone = Boolean(options.preservePhone); if (select.value === 'Ilya Shulyak') { phone.value = '(402) 413-1267'; phone.readOnly = true; } else { phone.readOnly = false; if (!preservePhone && (select.value === 'Other' || select.value === '')) phone.value = ''; } if (phone.dataset.format) formatFieldValue(phone); validateField(phone, false); }
  ...
  function handleChange(event) { const el = event.target; if (el.matches('select[data-other-target]')) toggleOtherForSelect(el); if (el.id === 'receiverContactName') syncReceiverContactName(); validateField(el, true); saveToStorage(); }
