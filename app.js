(() => {
  ...
  const state = { sigCanvas: null, sigCtx: null, isSigning: false, hasMark: false, activeSig: '1', signatureStorageFailed: false };
  ...
  function sigModalDone() { if (!state.hasMark || !state.sigCanvas) return; const dataURL = cropSigCanvas(state.sigCanvas); const preview = $('sigPreview' + state.activeSig); const box = $('sigBox' + state.activeSig); if (preview) { preview.src = dataURL; preview.style.display = 'block'; } if (box) box.classList.add('sig-has-data'); box?.classList.remove('field-missing'); stampSignatureDate(state.activeSig); saveToStorage(); const signatureSaved = safeLocalStorage((storage) => { storage.setItem(APP.signatureKeyPrefix + state.activeSig, dataURL); return true; }, false, { operation: 'save signature image' }); state.signatureStorageFailed = !signatureSaved; if (!signatureSaved) showStorageWarning(); $('sigModal')?.classList.remove('open'); }
  function clearSigBox(signatureId) { const id = String(signatureId); const preview = $('sigPreview' + id); if (preview) { preview.style.display = 'none'; preview.src = ''; } $('sigBox' + id)?.classList.remove('sig-has-data'); state.signatureStorageFailed = false; safeLocalStorage((storage) => storage.removeItem(APP.signatureKeyPrefix + id), null, { operation: 'clear signature image' }); }
  function restoreSignatures() { APP.signatureIds.forEach((id) => { const sig = safeLocalStorage((storage) => storage.getItem(APP.signatureKeyPrefix + id), null, { operation: 'read signature image' }); if (!sig) return; const preview = $('sigPreview' + id); const box = $('sigBox' + id); if (preview) { preview.src = sig; preview.style.display = 'block'; } if (box) box.classList.add('sig-has-data'); }); }
