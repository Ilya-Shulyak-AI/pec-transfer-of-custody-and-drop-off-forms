(() => {
  ...
  function clearSigBox(signatureId) { const id = String(signatureId); const preview = $('sigPreview' + id); const box = $('sigBox' + id); if (preview) { preview.style.display = 'none'; preview.src = ''; } if (box) box.classList.remove('sig-has-data', 'field-missing'); clearSignatureDate(id); safeLocalStorage((storage) => storage.removeItem(APP.signatureKeyPrefix + id)); }
